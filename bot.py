...
async def run_bot():
    print("--- [START] Avvio estrazione PROGRAMMI GARE (Espansione inclusa) ---")
    async with async_playwright() as p:
        ...
        for cat_id, filename in CATEGORIES.items():
            ...
            for status in STATUSES:
                ...
                # Filtri (usiamo i selettori di testo come nel tuo codice funzionante)
                ...
                # --- NUOVA LOGICA: ESPANSIONE LISTA ---
                print("-> Ricerca tasto 'Carica altri'...")
                while True:
                    btn_load_more = page.locator("button#btn-loadMore") # Verifica ID/Classe corretta
                    if await btn_load_more.is_visible():
                        print("   -> Trovato! Carico altri...")
                        await btn_load_more.click()
                        await asyncio.sleep(4) # Attesa per caricamento dati
                    else:
                ...
                print(f"-> Totale tornei trovati: {len(links)}")
                ...
                for link in links:
                    ...
                    for i in range(2):
                        ...
                        if await page.locator(f"#select-ordergame option:has-text('{data_target}')").count() > 0:
                            ...
                            await asyncio.sleep(4) # Attesa extra post-selezione
