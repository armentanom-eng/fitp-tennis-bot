...
async def run_bot():
    print("--- [LOG START] Avvio modalità resiliente ---")
    async with async_playwright() as p:
        ...
        page.set_default_timeout(45000) # Timeout generoso
        
        # 1. Recupero lista tornei (Filtri)
        ...
        # 2. Analisi tornei (Senza go_back)
        dati_giov, dati_open = [], []
        for url in urls:
            full_url = f"https://www.fitp.it{url}" if url.startswith('/') else url
            try:
                print(f"--- Analizzo: {full_url[-20:]} ---")
                await page.goto(full_url, wait_until="networkidle")
                
                # Conta quante categorie ci sono
                count = await page.locator("text=Dettaglio >").count()
                
                for i in range(count):
                    # Ri-selezioniamo il bottone ad ogni giro per evitare errori
                    ...
                    # INVECE DI GO_BACK, torniamo alla pagina del torneo
                    await page.goto(full_url, wait_until="networkidle")
