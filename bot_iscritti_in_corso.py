...
# Mappa delle categorie per sapere quale cliccare e dove salvare
# Cambia i data-id se necessario in base al sito
CATEGORIES = {
...
async def run_bot():
    print("--- [START] Avvio estrazione WEB (Separata per Categoria) ---")
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True, args=["--no-sandbox"])
        page = await browser.new_page()
        page.set_default_timeout(30000)
        
        # Iteriamo sulle categorie: il loop farà tutto due volte
        for cat_id, filename in CATEGORIES.items():
            ...
            # --- 2. FILTRO CATEGORIA (IL TRUCCO) ---
            print(f"-> Clicco filtro categoria: {cat_id}")
            await page.locator(f'a[data-id="{cat_id}"]').first.click()
            # Attendiamo il ricaricamento della lista (cruciale)
